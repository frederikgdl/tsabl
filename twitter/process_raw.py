...
def main():
    ...
    logging.info('Fetching ' + str(len(ids)) + ' tweets...')
    tweets = get_tweets(ids)
    logging.info('Fetched ' + str(len(tweets)) + ' tweets. '
        + str(len(ids) - len(tweets)) + ' tweets have been deleted.')
...
def print_intro():
    ...
    if args.extension:
        print('Raw tweets files extension:\t' + args.extension)
    else:
        print('Raw tweets data file:\t\t' + args.in_file)

    print()
    print('JSON output file:\t\t' + args.out_file_json)

    if args.out_file_text is not None:
        print('Text output file:\t\t' + args.out_file_text)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--extension', nargs='?', default=None,
                        help='set file extension to read multiple files \
                            (default: None)')
    ...
    if args.extension:
        for fname in os.listdir(args.dir):
            ...
            logging.critical('\nCould not find files with extension '
                + args.extension)
            sys.exit(1)
